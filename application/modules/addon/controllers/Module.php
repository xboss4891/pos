...
// Check if the index exists in $succe before accessing it
                            // if (isset($succe[$i]) && !$this->db->table_exists($succe[$i])) {
...
/*----------ADD TO MODULE ------------*/
                ...
                if ($HmvcConfig[$directory]["_database"] === false) {
    ...
    private function create_module_json()
    {
        ...
        if (isset($HmvcMenu) && $HmvcMenu != null && sizeof($HmvcMenu) > 0) {
            ...
            foreach ($HmvcMenu as $moduleName => $moduleData) {
                $jsonvalue[] = ['label' => $moduleName, 'value' => $moduleName];
            }

            // Directory where the JSON file will be stored
            $directory = './my-assets/js/admin_js/json/';
...
// Check if the directory exists, and create it if it doesn't
            ...
            $cache_file = $directory . 'paddon.json';

            // Encode the JSON and save it
            $itemlist = json_encode($jsonvalue, JSON_PRETTY_PRINT);
