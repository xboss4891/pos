...
class Order extends MX_Controller
{
    ...
    public function pos_invoice()
    {
        ...
        $data['possetting2']     = $this->order_model->read('*', 'tbl_quickordersetting', ['quickordid' => 1]);
        $data['soundsetting']    = $this->order_model->read('*', 'tbl_soundsetting', ['soundid' => 1]);
        $settinginfo             = $this->order_model->settinginfo();
    ...
    public function pos_order($value = null)
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($cart = $this->cart->contents()) {
                ...
                $customerinfo = $this->order_model->read('*', 'customer_info', ['customer_id' => $this->input->post('customer_name', true)]);
                $mtype        = $this->order_model->read('*', 'membership', ['id' => $customerinfo->membership_type]);
    ...
    public function cancelitem()
    {
        ...
        foreach ($itemids as $sitem) {
            ...
            if (!empty($taxinfos)) {
                ...
                foreach ($taxinfos as $taxinfo) {
                    ...
                    $updatetax = [$fildname => $taxcolec->$fildname - $vatcalc];
                    $this->db->where('relation_id', $orderid);
                    ...
                    foreach ($getaddonsdatas as $getaddonsdata) {
                        ...
                        foreach ($taxinfos as $taxainfo) {
                            ...
                            if (!empty($getaddonsdata[$fildaname])) {
                                ...
                                $addonsupdatetax = [$fildname => $avtax];
                                $this->db->where('relation_id', $orderid);
            ...
            $updatebill  = ['total_amount' => $subtotal, 'discount' => $grdiscount, 'service_charge' => $fsd, 'VAT' => $fvat, 'bill_amount' => $fbillamount];

            $this->db->where('order_id', $orderid);
            $this->db->update('bill', $updatebill);

            $updateorderinfo = ['totalamount' => $fbillamount];
            $this->db->where('order_id', $orderid);
        ...
        if (empty($afterorderinfo)) {
            $updatetData = ['anyreason' => "All item no available", 'order_status' => 5, 'nofification' => 1, 'orderacceptreject' => 0];
            $this->db->where('order_id', $orderid);
            $this->db->update('customer_order', $updatetData);
        }
    ...
    public function printtoken()
    {
        ...
        $data['customerinfo'] = $this->order_model->read('*', 'customer_info', ['customer_id' => $singleorderinfo->customer_id]);

        if (!empty($singleorderinfo->table_no)) {
            $data['tableinfo'] = $this->order_model->read('*', 'rest_table', ['tableid' => $singleorderinfo->table_no]);
        } else {
            $data['tableinfo'] = '';
        }
    ...
    public function onlinellorder()
    {
        ...
        foreach ($list as $rowdata) {
            ...
            if (!empty($rowdata->shipping_type)) {
                $shipinfo = $this->order_model->read('*', 'shipping_method', ['ship_id' => $rowdata->shipping_type]);
            }
    ...
    public function otherupdateorder($id)
    {
        ...
        $data['possetting']      = $this->order_model->read('*', 'tbl_posetting', ['possettingid' => 1]);
        $data['taxinfos']        = $this->taxchecking();
        $mtype                   = $this->order_model->read('*', 'membership', ['id' => $data['customerinfo']->membership_type ?? '']);
    ...
    public function modifyoreder()
    {
        ...
        if (!empty($taxinfos)) {
            ...
            if ($multiplletaxvalue !== '0' && !empty($multiplletaxvalue)) {
    ...
    public function deletetocart()
    {
        ...
        $calvat       = $itemtotal * $settinginfo->vat / 100;
        $updatedprice = $calvat + $itemtotal - ($discount ?? 0);
        $postData     = [
            'order_id'    => $orderid,
    ...
    public function addtocartupdate()
    {
        ...
        if ($isgroup == 1) {
            ...
            if (empty($orderchecked)) {
                ...
                foreach ($adonsarray as $singleaddons) {
                    $totalaqty = intval($adonsqtyarray[$x]) + intval($adqty[$x]);
                    $finaladdonsqty .= $totalaqty . ',';
                    $x++;
                }
    ...
    {
        $data['orderinfo']    = $this->order_model->read('*', 'customer_order', ['order_id' => $orderid]);
        $data['paymentinfo']  = $this->order_model->read('*', 'paymentsetup', ['paymentid' => $paymentid]);
        $paymentinfo          = $this->order_model->read('*', 'paymentsetup', ['paymentid' => $paymentid]);
        $data['customerinfo'] = $this->order_model->read('*', 'customer_info', ['customer_id' => $data['orderinfo']->customer_id]);
        $customer             = $this->order_model->read('*', 'customer_info', ['customer_id' => $data['orderinfo']->customer_id]);
        $bill                 = $this->order_model->read('*', 'bill', ['order_id' => $orderid]);
        $data['billinfo']     = $this->order_model->read('*', 'bill_card_payment', ['bill_id' => $bill->bill_id]);
    ...
    {
        $this->permission->method('ordermanage', 'read')->redirect();
        $billinfo    = $this->order_model->read('*', 'bill', ['order_id' => $orderid]);
        $orderinfo   = $this->order_model->read('*', 'customer_order', ['order_id' => $orderid]);
    ...
    public function itemacepted()
    {
        ...
        $itemids     = explode(',', $itemid);
        $varientids  = explode(',', $varient);
        $itemidsv    = array_values(array_map('trim', $itemids));
        $varientidsv = array_values(array_map('trim', $varientids));
        $i           = 0;
    ...
    public function itemisready()
    {
        ...
        $varient     = $this->input->post('varient', true);
        $status      = $this->input->post('status', true);
        $updatetData = ['food_status' => $status];
        $this->db->where('order_id', $orderid);
    ...
    public function showpaymentmodal($id, $type = null)
    {
        ...
        $array_id              = ['order_id' => $id];
        $order_info            = $this->order_model->read('*', 'customer_order', $array_id);
        $customer_info         = $this->order_model->read('*', 'customer_info', ['customer_id' => $order_info->customer_id]);
    ...
    public function mergemodal()
    {
        ...
        $customer_info       = $this->order_model->read('*', 'customer_info', ['customer_id' => $data['order_info'][0]->customer_id]);
        $data['membership']  = $customer_info->membership_type;
        $data['customerid']  = $customer_info->customer_id;
        $settinginfo         = $this->order_model->settinginfo();
        $data['settinginfo'] = $settinginfo;
    ...
    public function changeMargeorder()
    {
        ...
        $mydata['cashierinfo']      = $this->order_model->read('*', 'user', ['id' => $billinfo->create_by]);
        $mydata['customerinfo']     = $this->order_model->read('*', 'customer_info', ['customer_id' => $mydata['billinfo'][0]->customer_id]);
        $mydata['billdate']         = $billinfo->bill_date;
        $mydata['tableinfo']        = $this->order_model->read('*', 'rest_table', ['tableid' => $mydata['billinfo'][0]->table_no]);
    ...
    public function showsuborder($num = 1)
    {
        $this->permission->method('ordermanage', 'read')->redirect();
        $orderid                   = $this->input->post('orderid');
        $array_biil_id             = ['order_id' => $orderid];
    ...
    public function paymultiplsub()
    {
        ...
        foreach ($payamonts as $payamont) {
            $paidamount = $paidamount + $payamont;
            $data_pay   = ['paytype' => $paytype[$i], 'cterminal' => $cterminal[$i], 'mybank' => $mybank[$i], 'mydigit' => $mydigit[$i], 'payamont' => $payamont];
    ...
    public function posprintdirectsub($id)
    {
        ...
        $data['customerinfo'] = $this->order_model->read('*', 'customer_info', ['customer_id' => $order_sub->customer_id]);

        $data['billinfo']      = $this->order_model->billinfo($order_sub->order_id);
        $data['cashierinfo']   = $this->order_model->read('*', 'user', ['id' => $data['billinfo']->create_by]);
        $data['mainorderinfo'] = $this->order_model->read('*', 'customer_order', ['order_id' => $order_sub->order_id]);
        $data['tableinfo']     = $this->order_model->read('*', 'rest_table', ['tableid' => $data['mainorderinfo']->table_no]);
        $settinginfo           = $this->order_model->settinginfo();
    ...
    public function cashregister()
    {
        ...
        if (empty($checkuser)) {
            ...
            if ($openamount->closing_balance ?? '0.000') {
                $data['openingbalance'] = $openamount->closing_balance ?? 0;
            } else {
    ...
    public function addsound()
    {
        ...
        $data['soundsetting'] = (object) $postData = [
    ...
    {
        $data['quickordersetting'] = $this->order_model->read('*', 'tbl_quickordersetting', ['quickordid' => 1]);
        header('Content-Type: text/javascript');
    ...
    {
        $soundinfo     = $this->order_model->read('*', 'tbl_soundsetting', ['soundid' => 1]);
        $possetting    = $this->order_model->read('*', 'tbl_posetting', ['possettingid' => 1]);
        $settinginfo   = $this->order_model->settinginfo();
