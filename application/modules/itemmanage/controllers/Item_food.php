...
#-------------------------------#
        if ($this->form_validation->run()) {
            ...
            if (!$this->upload->do_upload('picture')) {
                ...
                $image_sizes = ['big' => [555, 370], 'medium' => [268, 223], 'small' => [116, 116]];
...
/****************end*********************/
        ...
        if (!empty($_FILES["userfile"]["name"])) {
            ...
            if ($datacount > 1) {
                ...
                for ($i = 1; $i < $datacount; $i++) {
                    ...
                    if ($newstatus == "active") {$acstatus = 1;} else { $acstatus = 0;}
    ...
    public function downloadformat()
    {
        ...
        $rowCount  = 2;
        $arrayfood = [
            ["category" => "Demo", "parent" => "", "kitchen" => "Italian", "item" => "Dosa", "description" => "food description", "status" => "Active", "varient" => "regular", "price" => 60], 
            ["category" => "Demo1", "parent" => "Demo", "kitchen" => "Italian", "item" => "Dosa2", "description" => "food description", "status" => "Active", "varient" => "Small", "price" => 50]
        ];

        foreach ($arrayfood as $row) {
            $sheet->SetCellValue('A' . $rowCount, $row['category']);
            $sheet->SetCellValue('B' . $rowCount, $row['parent']);
            $sheet->SetCellValue('C' . $rowCount, $row['kitchen']);
            $sheet->SetCellValue('D' . $rowCount, $row['item']);
            $sheet->SetCellValue('E' . $rowCount, $row['description']);
            $sheet->SetCellValue('F' . $rowCount, $row['status']);
            $sheet->SetCellValue('G' . $rowCount, $row['varient']);
...
/****************end*********************/
    ...
    public function checkfood()
    {
        ...
        foreach ($product_info as $value) {
            $json_product[] = ['label' => $value['ProductName'] . '_' . $value['variantName'], 'value' => $value['ProductsID'], "varientid" => $value['variantid'], "variantName" => $value['variantName'], "ProductName" => $value['ProductName'], "price" => $value['price']];
        }
